// import hotelList from "./hotel.js";
// const hotelList = require("./hotel.js");  //this code imports the hotel.js .
...
let actualChoice = document.querySelector(".dropbtn");
let icon = document.querySelector(".icon");

function countryChoice(){
    let btn = this;
    actualChoice.innerHTML = this.innerHTML;
    actualChoice.appendChild(icon);
}

for(single of choice){
    single.addEventListener("click", countryChoice);
}

let left_arrow = document.querySelector(".left-arrow");
let right_arrow = document.querySelector(".right-arrow");
let pages = document.querySelectorAll(".number");
let pageNum = 1;
left_arrow.setAttribute("disabled","disabled");

function changePage(){

    for(single of pages){
        if(single.classList.contains("btn")){
            single.classList.remove("btn");
            single.classList.remove("btn-success");
            single.classList.remove("rounded-circle");
            single.classList.remove("me-5");
        }   
    }
    ...
    btn.style.transform = "skew(0deg)";
    btn.classList.add("me-5")
    pageNum = parseInt(btn.id);
    ...
    if(btn.id == "1"){
        ...
        right_arrow.setAttribute("disabled","disabled");
...
function nextPage(){
    ...
    let callpage = document.getElementById(`${pageNum}`);
    
    arrowChangePage(callpage);
...
for(single of pages){
    single.addEventListener("click", changePage);
}

right_arrow.addEventListener("click", nextPage);
left_arrow.addEventListener("click", previousPage);


let contentDiv = document.querySelector(".content");
let contentAllDiv = document.querySelectorAll(".card");
let count = 0;
...
function createContent(element) {
  ...
  newDiv.classList.add("border-white");
  newDiv.style.width = "18 rem";
  newDiv.innerHTML = `
  <img src="${element.photo1}" class="card-img-top rounded-4 mb-4 mt-4" alt="...">
  <div class="card-body d-flex flex-column align-items-start p-0">
    <h5 class="card-title fs-4 fw-bold">${element.hotel_name}</h5>
    <h6 class="card-subtitle mt-3 mb-2 text-body-secondary">
      ...
      <div class="reviews">
          ...
          4.9 (1,390 Reviews)
          ${element.rating_average} (${element.number_of_reviews} Reviews)
      </div>
    </h6>
    <p class="card-text"><span class="card-title fw-bold fs-4">$ ${element.rates_from}</span>/Person</p>
  </div>`

  contentDiv.appendChild(newDiv);
  contentAllDiv = document.querySelectorAll(".card");
...
function filterCities(){
  contentAllDiv.forEach(element => {
    element.style.display = "none";
  });
  
  let value = input.value.toUpperCase();
  
  hotelList.forEach(function (element){
    if(element.city.toUpperCase() === value){
      createContent(element);
    }
    else if(element.country.toUpperCase() === value){
      createContent(element);
    }
    else if(element.hotel_name.toUpperCase() === value){
      createContent(element);
    }
  });
  input.value = "";
...
hotelList.every(createContent);
count = 0;
input.addEventListener("change", filterCities);

let allFilters = document.querySelectorAll(".countryFilter");
for(single of allFilters){
  single.addEventListener("active", filterCountries);
  single.labels.addEventListener("click", filterCountries);
}
